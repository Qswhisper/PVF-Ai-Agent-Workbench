...
- 中文实体名默认按完整名称字段做字面包含搜索，自动兼容名称前后缀、跨行 token、常见全角/半角标点和 Cn/Tw 读取；已知类别的命中会同时附带登记 ID，不需要再逐条反查。多个名称、多个同表 ID 或多个尚未确认的路径都可共用一次 PVF 会话，宽泛结果过多时会提示改用用户原话里的具体词。
...
普通脚本里的完整中文名称或描述现在支持单行、多行和受控批量修改。同一句完整文字重复时，可以用紧邻的目标原文联合定位；多个同构部位连相邻内容也相同时，还可把每条变化限定到原始读回中的精确块，只改目标部位。范围数量、边界、内容和位置会随预演及正式生成一起核对，缺失、重叠、越界或漂移都会停止。批量必须先声明范围内准确数量；参数与中文联动时会拆成同文件的独立变化，再统一检查最终结果。同一文件的多条文字会合并处理，不逐条重复重建。简体和繁体都会先经临时输出和独立解析验证。`.str`、StringLink 引用文字、部分中文、按出现序号或部位简称猜位置、数量不明确的批量和无法保存的字符仍会被阻止。

普通读取只适合查看，中文可能已经转成简体，不能直接复制成修改原文。准备 change-set 时必须对同一路径重新读取原始 token；若显示文字造成零命中，工作台会给出原始读取编码并安全停止，不会自动穷举繁简或跨编码写入。

普通配置脚本中的完整数字或英文参数修改已覆盖 `.wdm`、`.twn`、`.rgn`、`.mm`、`.cre`、`.npc` 和 `.msn`。新增 `.co`、`.lst`、`.nut`、`.sqr`、`.str`、`.wdm` 也有受控支持：change-set 必须提交匹配的 `writeProof`，工作台会重新检查格式、登记冲突、脚本结构、临时写出或编码往返，并独立读回输出；既有 `.co/.lst/.nut/.sqr/.str` 的普通修改仍被阻止。新增世界地图必须同时闭合 registry、UI、dungeon、town/region 入口，静态通过不等于客户端或实机通过。
