...
普通脚本里的单个中文名称或描述现在可以按安全流程修改，简体和繁体目标都支持：工作台会核对目标实际编码，先用临时文件验证，再按同一编码独立检查文字是否精确保留，通过后才允许生成新的 PVF。两种读法冲突时不会把明显乱码误报成文件损坏。`.str`、StringLink 引用文字、批量中文替换和无法无损保存的字符仍会被阻止。提示会优先使用通俗中文，不要求用户理解编码术语。
