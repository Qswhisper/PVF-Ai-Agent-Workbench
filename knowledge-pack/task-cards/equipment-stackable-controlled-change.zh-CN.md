...
5. 只改目标字段，不做顺手批量重排。
6. 含中文字符串或 StringLink 的文件，数字字段最小替换必须使用目标确认的 `Cn` 或 `Tw` 安全编码路线；普通脚本的完整中文名称/描述（可多行，批量需精确计数）可走 `verified-inline-text` 专项验证，重复完整文本可由同次原始读回的紧邻前后上下文精确定位，参数与中文拆为同路径变化，StringLink 与 `.str` 显示文本仍保持只读。
7. 保存到显式输出 PVF。
