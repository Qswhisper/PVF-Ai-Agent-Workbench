...
3. 普通读取只作浏览；其 `textUsage.safeForChangeSetSource=false` 表明文字可能已简体化、布局也可能被整理，不能复制到 change-set。对同一路径用 `pvf-read read --raw`（或 `read-batch --raw`）取得与写入校验一致的完整反引号 token（可含真实多行）。未明确指定编码时，原始读取只读比较 Cn/Tw，并在 `textUsage.automaticEncodingSelection` 中返回每个文件的 `selectedEncoding`；仅明显更干净时才改选，不做逐段猜测。使用 `textWriteMode: "verified-inline-text"` 和该 `pvfEncoding=Cn` 或 `pvfEncoding=Tw`。只改一处时用 `replaceAll=false`；若完整文字重复，从同一次原始读回复制紧邻目标的 `contextBefore` 和/或 `contextAfter`。若同构块连相邻上下文也重复，再从该次原始读回复制区间头尾，填写 `scope.startText`、`scope.endText`、正整数 `scope.expectedRanges`；目标和上下文必须完全位于区间内部。需要批量时用 `replaceAll=true` 并填写区间内的精确 `expectedOccurrences`。
   `.dgn [tower dialog]` 只识别“整数层号 → 完整对话 token → `` `on start` ``/`` `on complete` ``”这一严格记录；只改完整对话，不改事件 token。结构证据会绑定到临时往返与正式生成。
...
同文件有一条非法变化时，合法变化会显示 `FILE_CHANGE_BATCH_BLOCKED` 和 `blockedByChangeId`，表示它们尚未进入临时往返，而不是自身需要补 `CN_TEXT_ROUNDTRIP_REQUIRED`。按 `agentHandoff.sameFileBatchRecovery` 修正或移除指定阻断项，其余合法变化保持原样后重新 validate 与 dry-run。


## 活动介绍的窄范围写入

已定位的 `event/eventlistwindow.evt` 可按安全文件“活动列表成对介绍文字”处理：只改结构完整、原文相同的列表简介和弹窗正文，或成对单行标题，使用同一项精确计数的完整文字替换。日期、URL、按钮及其他 EVT 文件仍不开放。明确新增独立说明时，仅按安全文件“独立静态活动说明追加”闭合末尾单条结构，随后本地化占位文字；不能任意修改活动结构。生成后独立读回，安装后核对换行、正文可达和窗口溢出。不能把本能力当作全部活动系统文字写入许可。

## Explicit stackable name detachment

For an explicitly requested item rename, the narrow verified-stringlink-detach exception permits one complete .stk [name] StringLink to become an independent literal. Read knowledge-pack/task-cards/stackable-stringlink-detach-controlled-change.zh-CN.md (relative to the Workbench root). Shared localization files remain protected; ordinary text mode still rejects StringLink. This exception supersedes blanket StringLink read-only wording only for this route.
