...
2. 使用 `workbench.bat client-pvf preview --profile <name> --apply-manifest <file>`。预览同时核对客户端当前版本是否等于本轮生成的输入版本；不要手工复制或直接覆盖客户端文件。
...
- 最初受保护源、独立候选输出与客户端 `Script.pvf` 发生碰撞。多轮累积时，本轮只读输入可以是客户端里已部署的上一轮版本；预览会用哈希确认它确实一致，且不会把它当作受保护源或候选输出直接覆盖。
- profile 未启用、没有客户端目录，或目标不是客户端根目录的 `Script.pvf`。
- profile 的 `sourcePvf` 与 apply 清单记录的最初受保护源不一致，源 PVF 生成输出后发生变化，或预览后输出 PVF、客户端当前 PVF、profile、manifest 发生变化。
