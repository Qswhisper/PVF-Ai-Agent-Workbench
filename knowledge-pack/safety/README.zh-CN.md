...
- 新增 `.wdm`、`.co`、`.lst`、`.nut`、`.sqr`、`.str` 不再是能力级“永远不能做”，但也不属于普通 `write-file`：必须在 change-set 中提交匹配的 `writeProof`，由工作台对目标格式、目标路径冲突、脚本结构、临时写出或编码往返和独立读回重新核验。缺证据或任一核验失败仍保持阻断。已有 `.co/.lst/.nut/.sqr/.str` 的普通修改继续阻断；已有 `.lst` 只有明确的 `registry-lifecycle` 新增行并通过冲突/引用闭合检查才可进入专用路线。已有 `.wdm` 中只包含数字、英文和常见符号的既有字段仍可走原始 token 最小预演。
