...
| `.dgn [tower dialog]` 对话 token | 整数层号后的完整可见对话，下一 token 必须是 `on start` 或 `on complete` | 只开放完整对话 token；事件 token 和其他结构不获得文字写权限 |
...
- `.dgn [tower dialog]` 使用结构化容器证明：标签、整数层号、完整对话 token 和紧随的 `` `on start` ``/`` `on complete` `` 必须同时存在。证明记录层号、事件、路径与 SHA256；任何一项漂移都失败关闭。
- `FILE_CHANGE_BATCH_BLOCKED` 表示当前变化自身静态检查已通过，但同文件的 `blockedByChangeId` 先阻断了整批，因此没有执行临时往返。它与 `CN_TEXT_ROUNDTRIP_REQUIRED`（本应执行但缺少证明）含义不同。
