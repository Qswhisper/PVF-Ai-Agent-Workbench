...
- 普通脚本中允许标签下的完整反引号文本（可含真实多行），可使用 `verified-inline-text` 和目标确认的 `Cn` 或 `Tw`；重复完整文本可由同次原始读回中的紧邻前后上下文联合定位，锚定后仍须精确命中，选择器和位置写入核验绑定且不放宽其他保护；批量必须声明准确命中数。预演隔离写出最终同文件结果，独立 TypeScript 解析器精确读回，并证明所有旧字符串表条目保持不变；明显乱码或任一检查失败则不给 approval code。
