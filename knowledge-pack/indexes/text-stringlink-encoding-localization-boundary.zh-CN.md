...
- 任意中文写入流程都安全；只有受控验证模式覆盖的完整内联文本（可多行、批量需精确计数）具有机器往返证据。
...
- 普通脚本中允许标签下的完整反引号文本（可含真实多行），可使用 `verified-inline-text` 和目标确认的 `Cn` 或 `Tw`；重复完整文本可由同次原始读回中的紧邻前后上下文联合定位。同构块仍无法区分时，普通参数和完整文字变化可共同使用精确 `scope.startText/endText/expectedRanges`，只在非重叠区间内部计数。目标、上下文、范围字段哈希、范围位置及内容哈希进入核验绑定；边界写入、标记注入、缺失、重叠、越界或漂移均阻断，且不放宽其他保护。批量必须声明区间内准确命中数。预演隔离写出最终同文件结果，独立 TypeScript 解析器精确读回，并证明所有旧字符串表条目保持不变；明显乱码或任一检查失败则不给 approval code。
- 普通读取只是可能简体化或整理布局的阅读视图，`textUsage.safeForChangeSetSource=false`；修改前必须对同一路径使用 `--raw` 和返回的实际选择编码。零命中诊断能区分阅读显示文本误用与声明编码不一致，但只要求重新原始读取，不自动改字、不跨编码写入。
- 参数/结构与中文联动时拆为同一路径独立变化，统一验证最终文件。彼此独立时优先先做普通结构改动；若删除结构必须先清空其中的完整中文，则保留 change-set 数组顺序，允许“完整文字清空 → 删除文字残留结构 → 删除参数块”。无论采用哪种顺序，同一路径多条安全文字仍只重建一次字符串表并修改一次脚本，每一步精确命中且最终结果通过隔离临时输出复查。该证据不扩展到 `.str`、StringLink 显示文本、名称表、部分中文 token、未精确计数的批量或无法无损编码字符。
