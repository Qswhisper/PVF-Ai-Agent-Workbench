...
- 真正写出前会先预演；普通改动只检查，中文改动会用临时文件验证并立即清理。确认后生成独立 PVF，并自动备份和复查。
...
普通脚本里的单个中文名称或描述可以按安全流程修改，简体和繁体目标都支持：工作台会核对目标实际编码，先用临时文件生成，再由另一套解析器按同一编码精确检查，通过后才允许正式生成。两种读法冲突时不会把明显乱码误报成文件损坏。`.str`、StringLink 引用文字、批量中文替换和无法无损保存的字符仍会被阻止。这些检查由工作台自动处理，不会增加新手配置。
